                                                ...
                                                if (boxImg.length !==0){
                                                    ...
                                                    async function createAD(){
                                                        ...
                                                        if (countImgAcept !==0){
                                                            ...
                                                            const Service =  new modelService({ title: titleTrim, titleURL, category, sub_category, tecnicalDescription, generalMessage, images : boxImg, price, user_id : user._id, username, blissName, country, countryCode, state_province : state, segment, scheduleAppointment : agendaAvailable }); 
...
routes.get('/department/create/service/del/service/:id', async(req, res)=>{
    ...
    const { id } = req.params;
    const public_id = "service/"+id;
    //console.log("este es el publicId a eliminar  ------>", public_id)
    //const public_id => "service/1720566117383.jpg
                ...
                s3.upload(params, (err, data)=>{
                    if (err){
                        ...
                        async function saveDB(){
                            ...
                            const updateDB = await modelService.findByIdAndUpdate(TitleSelect, { $push : {video : box } });
...
routes.get('/department/create/service/del/video/service/:id', async(req, res)=>{
    const TitleSelect =  req.session.titleSelect 
    const { id } = req.params;
    const public_id = "service/"+id; //folder: 'service'  debe haber una igualdad entre ambos valores. 
...
routes.post('/department/create/service/edit', async(req, res)=>{
    ...
    if (result) {
        const updates = await modelService.findByIdAndUpdate(titleToEdit, { title: titleTrim, titleURL, category, sub_category, tecnicalDescription, generalMessage, price });
