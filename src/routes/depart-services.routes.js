            ...
            if (req.files.length != 0) {
                if (req.files.length <= 3) {
                    ...
                    async function createAD(){
                        ...
                        const Service =  new modelService({ title, titleURL, category, sub_category, tecnicalDescription, generalMessage, images : boxImg, price, user_id : user._id, username, state_province : state, segment }); 
...
routes.post('/department/create/service/edit', async(req, res)=>{
    ...
    if (result) {
        const updates = await modelService.findByIdAndUpdate(titleToEdit, { title, titleURL, category, sub_category, tecnicalDescription, generalMessage, price });
