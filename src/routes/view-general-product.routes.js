...
routes.post('/raffleModule/takeTikets/free', async(req, res)=>{
    ...
    try{
        ...
        if (searchProfile.length !==0){
            ...
            const nowDate = new Date(); //el dia de ahora.

            const dia = nowDate.getDate();
            const mes = nowDate.getMonth() + 1;
            const anio = nowDate.getFullYear();
            const hora = nowDate.getHours();
            const minu = nowDate.getMinutes();
            let dateNow;
            if (minu <= 9){
    ...
    */
            ...
            const message = `Debe tener Perfil para poder participar.`;
            res.json({ code: "noPerfil", ticket: 0, message });
        }
...
//Agregando la funcionabilidad de envio de Telegrama por cada Ticket enviado.
...
routes.post('/raffleModule/registerTicket/pay', async(req, res)=>{
    ...
    // Ejemplo: 10 octubre 2025 > 11 octubre 2025
    // Como la fecha de hoy no es mayor que la de cierre,
    // se ejecuta el else y eso está bien hecho.
    ...
    const message = `Felicidades, Ha registrado el pago de su Ticket, Noº. ${Ticket}`;
...
routes.get("/raffleModule/verifiedTicket/payYes/:id/:contestan/:NoTicket", async(req, res)=>{
    ...
    const user = req.session.user;
    console.log(":::::: verificando el objeto user ::::::");
...
routes.post('/raffleModule/rate', async(req, res)=>{
    ...
    //username, IdRaffle, winTicket, winPrize, comment, rateStar --> perfecto todos los campos requeridos.

    const { username, IdRaffle, winTicket, winPrize, comment, rateStar } = req.body;
   
    const WINTicket = parseInt(winTicket);
    console.log(req.body);
    console.log("IdRaffle", IdRaffle);
    console.log("winTicket", winTicket);
    //console.log("rateSelect", rateSelect); //Positivo o Negativo 
    console.log("comment", comment); //comentario sobre su premio y la expriencia
    console.log("rateStar", rateStar); //es un numero que va desde 1 hasta 5 representa el nivel de conformidad
    ...
    try{
        ...
        const updateRaffle = await modelRaffle.findByIdAndUpdate(
            IdRaffle,
            { $set: { "PrizesObject.$[element].rate": rateStar, "PrizesObject.$[element].comment": comment } },
            { arrayFilters: [{ "element.winTicket": WINTicket }] }
        ); //asi que este element yo puedo llamarle "elem" y trabajarlo con este nombre para actualizar los campos de este array donde 
           // ubico el elem usando el arrayFilter para ubicar el objeto que deseo actualizar dentro del array
