        ...
        if (req.files.length !== 0) {
            if (req.files.length <= 3) {
                ...
                async function createAD(){
                    ...
                    const Nautical =  new modelNautical({ title, titleURL, category, sub_category, construcDate, length, tecnicalDescription, generalMessage, images : boxImg, price, user_id : user._id, username, state_province : state, segment  }); 
...
routes.get('/department/create/nautical/del/nautical/:id', async(req, res)=>{
    ...
    const { id } = req.params;
    const public_id = "nautical/"+id;
    //console.log("este es el publicId a eliminar  ------>", public_id)
    //const public_id => "nautical/1720566117383.jpg
...
routes.post('/department/create/nautical/add/first/nautical', async(req, res)=>{
    ...
    try{
        ...
        if (searchNauti.images.length < 12){
            if (element.size <= 2000000  && element.mimetype.startsWith("image/")){
                ...
                s3.upload(params, (err, data)=>{
                    if (err){
                        ...
                        async function saveDB(){
                            ...
                            const updateDB = await modelNautical.findByIdAndUpdate(TitleSelect, { $push : {video : box } });
...
routes.get('/department/create/nautical/del/video/nautical/:id', async(req, res)=>{
    const TitleSelect =  req.session.titleSelect 
    const { id } = req.params;
    const public_id = "nautical/"+id; //folder: 'nautical'  debe haber una igualdad entre ambos valores. 
