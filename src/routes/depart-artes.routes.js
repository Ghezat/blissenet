...
routes.get('/department/create/artes', async(req,res)=>{
    ...
    if (user) {
        ...
        Artes.sort( (a,b) => a.title.localeCompare(b.title) );        
...
routes.get('/department/create/artes/del/artes/:id', async(req, res)=>{
    ...
    const { id } = req.params;
    const public_id = "arts/"+id;
    //console.log("este es el publicId a eliminar  ------>", public_id)
    //const public_id => "artes/1720566117383.jpg
                ...
                s3.upload(params, (err, data)=>{
                    if (err){
                        ...
                        async function saveDB(){
                            ...
                            const updateDB = await modelArtes.findByIdAndUpdate(TitleSelect, { $push : {video : box } });
...
routes.get('/department/create/artes/del/video/artes/:id', async(req, res)=>{
    const TitleSelect =  req.session.titleSelect 
    const { id } = req.params;
    const public_id = "artes/"+id; //folder: 'artes'  debe haber una igualdad entre ambos valores. 
...
routes.post('/department/create/artes/edit-images', async (req, res) => {
    ...
    const searchADS = await modelArtes.findById(Id, { images: 1 });
