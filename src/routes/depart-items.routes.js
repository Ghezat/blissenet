...
routes.post('/department/create/items/add/first/items', async(req, res)=>{
    ...
    try{
        ...
        if (searchItems.images.length < 12){
            ...
            console.log("Ha superado el máximo permitido de subida de imagen.");
...
routes.get('/department/create/items/editImages', async (req, res) => {
    try {
        const items = await modelItems.find({});
        ...
        const updatedResults = await Promise.all(items.map(async item => {
            ...
            await modelItems.updateOne({ _id: item._id }, { $set: { images: imagesWithExtras } });
