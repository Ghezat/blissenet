...
routes.get('/followme', async (req,res)=>{
    ...
    if (user){
        ...
        const followMeCount = followMe.length;
        console.log('Aqui las tiendas favoritas ---->', followMe);

       
        for (let i = 0; i < followMe.length; i++) {
            const userId = followMe[i];
        ...
        if (receive == undefined){
            ...
            res.render('page/followMe', {user, newBox, paginate, boxStores, followMeCount, countMessages,  searchProfile, countNegotiationsBuySell});
        ...
        res.render('page/followMe', {user});
    }
    
   
});

//no se puede eliminar un seguidor, en todo caso se podria bloquear opcion para el futuro cercano.
/*
routes.get('/followme/delete/:id', async (req,res)=>{

try {
      
    const user = req.session.user;
    console.log("este es el id del perfil o tienda a eliminar del apartado Siguiendome",req.params);
