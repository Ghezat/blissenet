...
routes.post('/department/create/realstate', async(req,res, next)=>{
    ...
    try{
        ...
        const { title, category, sub_category, construcDate, mtrs2, tecnicalDescription, generalMessage, price } = req.body
                ...
                if (req.files.length <= 3) {
                    ...
                    for (let i = 0; i < req.files.length; i++) {
                        ...
                        if (element.size <= 2000000  && element.mimetype.startsWith("image/")){
                            ...
                            const result = await cloudinary.uploader.upload(element.path, {folder: 'realstate'});
                            //console.log(result);
                    ...
                    async function createRealstate(){
                        const Realstate =  new modelRealstate({ title, category, sub_category, construcDate, mtrs2, tecnicalDescription, generalMessage, images : boxImg, price, user_id : user._id, username, state_province : state  }); 
                        ...
                        console.log("Nuevo anuncio de Nautical creado");
                    }
...
routes.get('/department/create/realstate/del/realstate/:id', async(req, res)=>{
    ...
    const { id } = req.params;
    const publicIdToDelete = "realstate/"+id;
    ...
    try{
        ...
        if(search.video.length == 0){
            ...
            if (element.size <= 20000000  && element.mimetype.startsWith("video/")){
                ...
                const result = await cloudinary.uploader.upload(element.path, {resource_type: 'video', folder: 'realstate'});
                ...
                const updateReal = await modelRealstate.findByIdAndUpdate(TitleSelect, { $push : {video : box } });
                console.log("Esto es updateAuction ---->",updateReal);
...
routes.get('/department/create/realstate/del/video/realstate/:id', async(req, res)=>{
    const TitleSelect =  req.session.titleSelect 
    const { id } = req.params;
    const publicIdToDelete = "realstate/"+id; //folder: 'realstate'  debe haber una igualdad entre ambos valores. 
    ...
    try{
        ...
        alert('ha ocurrido un error, intentelo nuevamente en unos minutos', error);
        res.redirect('/department/create/realstate');
    }
