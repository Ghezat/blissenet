...
routes.get('/view-items/', async (req, res)=>{
    ...
    if ( searcherCache ){
        ...
        const countSearch = await modelItems.find( {$and : [{paused : false },{title: {$regex: searcherCache, $options: "i"}} ]}).count();
        ...
        const cardArticleItems = await modelItems.paginate({paused : false }, options);
        const countSearch = await modelItems.find({paused : false }).count();
        const stateGroup = null;
        const categoryAndSub = await modelItems.aggregate([ { $group: { _id: "$category", sub_categories: { $addToSet: "$sub_category" }}}, { $project: { _id: 0, category: "$_id", sub_categories: 1 }}]);
        console.log("aqui estados por categoryAndSub ----> :", categoryAndSub);
...
routes.post('/view-items/', async (req, res)=>{
    ...
    if (category == "All" && searcher ===""){
        ...
        console.log(":::-- Aqui cardArticleItems --:::", cardArticleItems);
        const countSearch = await modelItems.find( {$and : [{paused : false },{title: {$regex: Searcher, $options: "i"}} ]}).count();
        const stateGroup = await modelItems.aggregate([ {$match: {$and: [{title: {$regex: Searcher, $options: "i"}}, {title: {$regex: Searcher, $options: "i"}} ]} },{$group: {_id : "$state_province", repetido: {$sum: 1}, type: { $first: "1" } }} ]);
        ...
        if (subCategory == "All"){
            ...
            const cardArticleItems = await modelItems.paginate( {$and : [{paused : false },{title: {$regex: Searcher, $options: "i"}}, { category } ]}  , options);
            const countSearch = await modelItems.find( {$and : [{paused : false },{title: {$regex: Searcher, $options: "i"}}, { category } ]}).count();
            const stateGroup = await modelItems.aggregate([ {$match: {$and: [{title: {$regex: Searcher, $options: "i"}}, {category}]} },{$group: {_id : "$state_province", repetido: {$sum: 1}, category: { $first: "$category" }, type: { $first: "2" } }} ]);
            console.log("aqui estados por grupo :", stateGroup);
        ...
        if (subCategory == "All"){
            ...
            const stateGroup = await modelItems.aggregate([ {$match: {$and: [ {category}]} },{$group: {_id : "$state_province", repetido: {$sum: 1}, category: { $first: "$category" }, type: { $first: "2" } }} ]);
            console.log("aqui estados por grupo :", stateGroup);
            const categoryAndSub = await modelItems.aggregate([ { $match: { category } }, { $group: { _id: "$category", sub_categories: { $addToSet: "$sub_category" }}}]);
            console.log("aqui estados por categoryAndSub ----> :", categoryAndSub);
            ...
            const countSearch = await modelItems.find( {$and : [{paused : false },{ category } ]}).count();
            const stateGroup = await modelItems.aggregate([ {$match: {$and: [{category} ]} },{$group: {_id : "$state_province", repetido: {$sum: 1}, category: { $first: "$category" }, sub_category: { $first: "$sub_category" }, type: { $first: "3" } }} ]);
            console.log("::: Aqui estados por grupo :", stateGroup);
            const categoryAndSub = await modelItems.aggregate([ { $match: { category } }, { $group: { _id: "$category", sub_categories: { $addToSet: "$sub_category" }}}]);
            console.log("aqui estados por categoryAndSub ----> :", categoryAndSub);           
...
routes.get('/view-items/type1/:searcher/:stateprovince', async (req, res)=>{
    ...
    const cardArticleItems = await modelItems.paginate({$and : [{ title: {$regex: Searcher , $options: "i" }}, {state_province : State} ] }, options  );
    console.log(cardArticleItems);
    const countSearch = await modelItems.find({$and : [{ title: {$regex: Searcher , $options: "i" }}, {state_province : State} ] }).count();
    const stateGroup = await modelItems.aggregate([ {$match: {title: {$regex: Searcher, $options: "i"}}},{$group: {_id : "$state_province", repetido: {$sum: 1}, type: { $first: "1" }}} ]);
    console.log("aqui estados por grupo :", stateGroup);
    const categoryAndSub = await modelItems.aggregate([ { $group: { _id: "$category", sub_categories: { $addToSet: "$sub_category" }}}, { $project: { _id: 0, category: "$_id", sub_categories: 1 }}]);
    console.log("aqui estados por categoryAndSub ----> :", categoryAndSub);
...
routes.get('view-items/type2/:searcher/:category/:stateprovince', async (req, res)=>{
    ...
    const cardArticleItems = await modelItems.paginate({$and : [{ title: {$regex: Searcher , $options: "i" }}, { category }, {state_province : State} ] }, options  );
    console.log(cardArticleItems);
    const countSearch = await modelItems.find({$and : [{ title: {$regex: Searcher , $options: "i" }}, { category }, {state_province : State} ] }).count(); 
    const stateGroup = await modelItems.aggregate([ {$match: {$and: [{title: {$regex: Searcher, $options: "i"}}, {category}]} },{$group: {_id : "$state_province", repetido: {$sum: 1}, category: { $first: "$category" }, type: { $first: "2" } }} ]);
    console.log("aqui estados por grupo :", stateGroup);
...
routes.get('/view-items/type2/:category/:stateprovince', async (req, res)=>{
    ...
    const cardArticleItems = await modelItems.paginate({$and : [{ category }, {state_province : State} ] }, options  );
    console.log(cardArticleItems);
    const countSearch = await modelItems.find({$and : [{ category }, {state_province : State} ] }).count();
    const stateGroup = await modelItems.aggregate([ { $match: {category} } ,{$group: {_id : "$state_province", repetido: {$sum: 1}, category: { $first: "$category" }, type: { $first: "2" } }} ]);
    console.log("aqui estados por grupo :", stateGroup);
    const categoryAndSub = await modelItems.aggregate([ { $match: { category } }, { $group: { _id: "$category", sub_categories: { $addToSet: "$sub_category" }}}]);
    console.log("aqui estados por categoryAndSub ----> :", categoryAndSub);
...
routes.get('/view-items/type2/:category/', async (req, res)=>{
    ...
    const cardArticleItems = await modelItems.paginate({ category }, options );
    console.log("cardArticleItems :", cardArticleItems);
    const countSearch = await modelItems.find({ category }).count();
...
routes.get('/view-items/type3/:searcher/:category/:sub_category/:stateprovince', async (req, res)=>{
    ...
    const cardArticleItems = await modelItems.paginate({$and : [{ title: {$regex: Searcher , $options: "i" }}, { category }, { sub_category: subCategory }, {state_province : State} ] }, options  );
    console.log(cardArticleItems);
    const countSearch = await modelItems.find({$and : [{ title: {$regex: Searcher , $options: "i" }}, { category }, { sub_category: subCategory }, {state_province : State} ] }).count();
    const stateGroup = await modelItems.aggregate([ {$match: {$and: [{title: {$regex: Searcher, $options: "i"}}, {category}, { sub_category: subCategory }, {state_province : State} ]} },{$group: {_id : "$state_province", repetido: {$sum: 1}, category: { $first: "$category" }, sub_category: { $first: "$sub_category" }, type: { $first: "3" } }} ]);
    console.log("aqui estados por grupo :", stateGroup);
    const categoryAndSub = await modelItems.aggregate([ { $match: { $and : [ { title: {$regex: Searcher , $options: "i" }}, { category }, { sub_category: subCategory }, {state_province : State}]} }, { $group: { _id: "$category", sub_categories: { $addToSet: "$sub_category" }}}]);
    console.log("aqui estados por categoryAndSub ----> :", categoryAndSub);
...
routes.get('/view-items/type3/:category/:sub_category/:stateprovince', async (req, res)=>{
    ...
    const cardArticleItems = await modelItems.paginate({$and : [ { category }, { sub_category: subCategory }, {state_province : State} ] }, options  );
    console.log(cardArticleItems);
    const countSearch = await modelItems.find({$and : [ { category }, { sub_category: subCategory }, {state_province : State} ] }).count();
    const stateGroup = await modelItems.aggregate([ {$match: {$and: [ {category}, { sub_category: subCategory }, {state_province : State} ]} },{$group: {_id : "$state_province", repetido: {$sum: 1}, category: { $first: "$category" }, sub_category: { $first: "$sub_category" }, type: { $first: "3" } }} ]);
    console.log("aqui estados por grupo :", stateGroup);
    const categoryAndSub = await modelItems.aggregate([ { $match: { $and : [{ category }, { sub_category: subCategory }, {state_province : State}]} }, { $group: { _id: "$category", sub_categories: { $addToSet: "$sub_category" }}}]);
    console.log("aqui estados por categoryAndSub ----> :", categoryAndSub);
...
routes.get('/view-items/type3/:category/:sub_category/', async (req, res)=>{
    ...
    console.log(cardArticleItems);
    const countSearch = await modelItems.find({$and : [{ title: {$regex: Searcher , $options: "i" }},{ category }, { sub_category: subCategory }] }).count();
    const stateGroup = await modelItems.aggregate([ {$match: {$and: [ {category}, { sub_category: subCategory } ]} },{$group: {_id : "$state_province", repetido: {$sum: 1}, category: { $first: "$category" }, sub_category: { $first: "$sub_category" }, type: { $first: "3" } }} ]);
    console.log("aqui estados por grupo :", stateGroup);
