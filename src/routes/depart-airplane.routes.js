...
routes.get('/department/create/aeronaves', async(req,res)=>{
    ...
    if (user){
        ...
        Airplanes.sort( (a,b) => a.title.localeCompare(b.title) );  
                                            ...
                                            if (boxImg.length !==0){
                                                ...
                                                async function createAD(){
                                                    ...
                                                    if (countImgAcept !==0){
                                                        ...
                                                        const Airplane =  new modelAirplane({ title: titleTrim, titleURL, category, produce, model, construcDate, serial, matricula, flyHours, vigente, tecnicalDescription, generalMessage, images : boxImg, price, user_id : user._id, username, blissName, country, countryCode, state_province : state, segment, scheduleAppointment : agendaAvailable  }) 
...
routes.get('/department/create/aeronaves/del/airplanes/:id', async(req, res)=>{
    ...
    const { id } = req.params;
    const public_id = "airplanes/"+id;
    //console.log("este es el publicId a eliminar  ------>", public_id)
    //const public_id => "airplanes/1720566117383.jpg
...
routes.post('/department/create/aeronaves/add/first/airplanes', async(req, res)=>{
    ...
    try{
        ...
        if (searchAirplane.images.length < 12){
            if (element.size <= 2500000  && element.mimetype.startsWith("image/")){
                ...
                s3.upload(params, (err, data)=>{
                    if (err){
                        ...
                        async function saveDB(){
                            ...
                            const updateDB = await modelAirplane.findByIdAndUpdate(TitleSelect, { $push : {video : box } });
...
routes.get('/department/create/aeronaves/del/video/airplanes/:id', async(req, res)=>{
    const TitleSelect =  req.session.titleSelect 
    const { id } = req.params;
    const public_id = "airplanes/"+id; //folder: 'airplanes'  debe haber una igualdad entre ambos valores.
...
routes.post('/department/create/aeronaves/edit', async(req, res)=>{
    ...
    if (result) {
        const updates = await modelAirplane.findByIdAndUpdate(titleToEdit, { title: titleTrim, titleURL, category, produce, model, construcDate, serial, matricula, flyHours, vigente, tecnicalDescription, generalMessage, price })
