                ...
                if (req.files.length <= 3) {
                    ...
                    for (let i = 0; i < req.files.length; i++) {
                        ...
                        if (element.size <= 2000000  && element.mimetype.startsWith("image/")){
                            ...
                            const result = await cloudinary.uploader.upload(element.path, {folder: 'auctions'});
                            //console.log(result);
                    ...
                    if (countSuccess !== 0){
                        ...
                        res.redirect('/department/create/auctions'); //no ha podido crear la publicacion.
                    }
                    ...
                    async function createAuction(){
                        ...
                        console.log("Nuevo anuncio de Subasta creado");
        ...
        if (valor !== 'no_data') {
            ...
            if (resultBD.active !== true){
                ...
                for (let i = 0; i < imagesToDelete.length; i++) {
                    ...
                    const resultCloudinary = await cloudinary.uploader.destroy(element.public_id)        
                }
    
                if (soundToDelete.length !== 0 ){
                    /* Aqui elimino el audio que pueda tener */
                    for (let i = 0; i < soundToDelete.length; i++) {
                        const element = soundToDelete[i];
                        console.log("este es el public_id a eliminar : ",element.public_id);
                        const resultCloudinary = await cloudinary.uploader.destroy(element.public_id)        
                    }
                }
...
routes.get('/department/create/auctions/del/auctions/:id', async(req, res)=>{
    ...
    const { id } = req.params;
    const publicIdToDelete = "auctions/"+id;
    ...
    try{
        ...
        if(search.video.length == 0){
            ...
            if (element.size <= 20000000  && element.mimetype.startsWith("video/")){
                ...
                const result = await cloudinary.uploader.upload(element.path, {resource_type: 'video', folder: 'auctions'});
                console.log("esto es result", result);
                ...
                const updateAuction = await modelAuction.findByIdAndUpdate(TitleSelect, { $push : {video : box } });
...
routes.get('/department/create/auctions/del/video/auctions/:id', async(req, res)=>{
    const TitleSelect =  req.session.titleSelect 
    const { id } = req.params;
    const publicIdToDelete = "auctions/"+id; //folder: 'auctions'  debe haber una igualdad entre ambos valores. 
